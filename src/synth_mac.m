...
static void synth_mac_mono_to_stereo_i16(const float *mono, int16_t *out, int n) {
    int i;
#if SHAKTI_USE_ACCELERATE
    float scaled[SYNTH_MAC_BUF];
    float scale = 32767.f;
    vDSP_vsmul(mono, 1, &scale, scaled, 1, (vDSP_Length)n);
    for (i = 0; i < n; i++) {
        float s = scaled[i];
...
static void synth_mac_audio_callback(void *user, AudioQueueRef q, AudioQueueBufferRef buf) {
    ...
    if (!synth_core_audio_running()) {
        memset(out, 0, (size_t)SYNTH_MAC_BUF * 2u * sizeof(int16_t));
    } else {
        float mono[SYNTH_MAC_BUF];
        synth_core_render(mono, SYNTH_MAC_BUF);
        synth_mac_mono_to_stereo_i16(mono, out, SYNTH_MAC_BUF);
    }
