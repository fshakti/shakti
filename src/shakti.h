...
/* Tagged-union value: common fields on every V; payload exclusive by ->t.
 *
 * Coexistence (must NOT share storage):
 *   - j vs _ht_cap: scalars/fn/input use j; vec/mat/list/dict/table use _ht_cap.
 *   - T_INPUT uses j (mode) and s (prompt) together.
 *   - T_SUBPROCESS uses j (PTY fd), n (pid, then 0/-1 after wait), bits (exit/signal).
 *   - T_FN uses j (body AST index) and params/defaults/closure together.
 *     Builtin wrappers (n == -1) store the name in s instead of params.
 *   - bits, owner_kind, map_reg apply across numeric/buffer types.
 *   - J/F/B stay distinct so a vec may hold a wide pointer and a packed B.
 */
struct V {
    int t;
    int rc;
    int64_t n;
    int bits;         /* lane width: bool=1; char=8; int/float 64 */
    int owner_kind;   /* V_OWNER_* */
    union {
        int64_t j;        /* bool/int/char scalar; date/time; T_INPUT mode; T_FN body idx; T_SUBPROCESS fd */
        uint32_t _ht_cap; /* vec capacity / matrix cols / list cap / dict hashtable cap */
    };
    void *map_reg;    /* IefsMapRegion* when V_OWNER_MAP_ALIAS */
    union {
        double f;                                         /* T_FLOAT */
        char *s;                                          /* T_STR / T_ERR / T_INPUT prompt; T_FN builtin name */
        struct { int64_t *J; double *F; unsigned char *B; }; /* vec/mat */
        V **L;                                            /* T_LIST */
        struct { V *keys, *vals; uint32_t *_ht; };        /* T_DICT / T_TABLE */
        struct { V *params, *defaults; Env *closure; };   /* T_FN */
    };
};
#if UINTPTR_MAX == UINT64_MAX
_Static_assert(sizeof(struct V) == 64, "struct V must stay 64 bytes");
#endif
...
enum {
    ...
    T_DATE = 6,
    T_ERR = 7,
    /* vecs together: IVEC first, LIST last so T_IVEC..T_LIST includes cvec */
    T_IVEC = 8,
    T_FVEC = 9,
    T_CVEC = 10,
    T_BVEC = 11,
    ...
    T_INPUT = 18,
    T_SUBPROCESS = 19,
    /* mats together: IMAT first, BMAT last so T_IMAT..T_BMAT includes cmat */
    T_IMAT = 20,
    T_FMAT = 21,
    T_CMAT = 22,
    T_BMAT = 23,
...
enum {
    ...
    OP_POW = 17,
    OP_SUB = 18,
    OP_MATMUL = 19,  /* reserved; '@' parses as N_EACH */
    OP_ASOF_COMMA = 20,
};
